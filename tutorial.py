...
def user_tutorial(root):
    ...
    g1 = Label(page2, image=img1).pack(side = 'left')

    img2 = PhotoImage(file=chordbuilder)
    g2 = Label(page3, image=img2).pack(side = 'left')

    img3 = PhotoImage(file=altbuild)
    g3 = Label(page4, image=img3).pack(side = 'left')

    img4 = PhotoImage(file=scalebuild)
    g4 = Label(page5, image=img4).pack(side = 'left')

    img5 = PhotoImage(file=tuningmenu)
    g5 = Label(page6, image=img5).pack(side = 'left')

    img6 = PhotoImage(file=optionsmenu)
    g6 = Label(labelframe1, image=img6).grid(column = 1, row = 1)

    img7 = PhotoImage(file=cdotdefault)
    g7 = Label(labelframe1, image=img7).grid(column = 1, row = 2)

    img8 = PhotoImage(file=cdotcolorblind)
    g8 = Label(labelframe1, image=img8).grid(column = 1, row = 3)

    img9 = PhotoImage(file=cshapedefault)
    g9 = Label(labelframe1, image=img9).grid(column = 1, row = 4)

    img10 = PhotoImage(file=helpmenu)
    g10 = Label(page8, image=img10).pack(side = 'left')

    img11 = PhotoImage(file=guitardisplay)
    g11 = Label(page9, image=img11).pack()
